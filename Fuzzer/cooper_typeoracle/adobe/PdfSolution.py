...
def pdf_cooper_data_prepare(pdf_dirpath, pdf_datapath):
    ...
    print("pdf_cooper_data_prepare return")
    return coopdata
    ...
    while True:
        ...
        for apiname, pdfname, identifers in mutation_info:
            ...
            if random.randint(0,4) > 3:
                jscode2 = script_generator(apiname)
            gen = generate.Generator()
            gen.clean_status()
            tmp = gen.create(SNUM)
            ...
            output_name = '{}.pdf'.format(generated_cnt)
            # Modify KMFL end
...
# Modify KMFL start
def generate_new_test(pdf_dirpath, out_dirpath, cooper_data, jscode, file_name):
    assert isinstance(cooper_data, CooperDataCollection)
    generated_cnt = 0
    want_cnt = 1
    while True:
        ...
        # input()
        for apiname, pdfname, identifers in mutation_info:
            jscode2 = pdf_js_generator(apiname)
            jscode = 'try{spell.available}catch(e){};\n'+jscode2+'\n'+jscode+'\ncloseDoc(1);'
            mutated_pdf = pdf_native_mutation(pdf_dirpath, pdfname, identifers, cooper_data, jscode)
            if not pdf_add_js(mutated_pdf, jscode):
                print 'ERROR: when add js code.'
            output_path = os.path.join(out_dirpath, file_name)
            with open(output_path, 'wb') as outfd:
                mutated_pdf.write(outfd)
            print 'Successfully generate {}.'.format(file_name)
